...
{
	cout << "DEBUG: using circuit file - " << filename << endl;
...
CircuitInput0CircuitBase.resize(Env::circuit().evl_inp_cnt()*2);
	...
	//daveti: change the path?
	//init_private("./input/inp.txt");
	cout << "DEBUG: using backend file - " << HYBRID_YAO_BACKEND_FILE << endl;
	init_private(HYBRID_YAO_BACKEND_FILE);
